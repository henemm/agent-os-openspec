...
def marker_block(name: str, version: str) -> str:
    ...
    return (
        ...
        "\n"
        "Die erste Zeile sagt, wer am Zug ist, und steht in genau einer von zwei Formen: "
        "`❗ Du: …`, wenn der PO den Schritt tippen oder eine Entscheidung treffen muss "
        "(bei Dringendem `‼️` statt `❗`) — oder `ℹ️ Nichts zu tun: <Claude arbeitet / "
        "wartet auf …> — danach: /<befehl> #<N>`, wenn Claude gerade selbst arbeitet "
        "oder auf ein Ergebnis wartet. So muss der PO nie raten, ob etwas von ihm "
        "erwartet wird.\n"
        "\n"
